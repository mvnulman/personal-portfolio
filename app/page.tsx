  ...
  if (!process.env.HYGRAPH_URL || !process.env.HYGRAPH_TOKEN) {
    ...
    try {
      ...
      const [userResponse, reposResponse] = await Promise.all([
        ...
        axios.get(
          `https://api.github.com/users/${GITHUB_USERNAME}/repos?sort=stars&per_page=10&type=public`,
          { headers },
        ),
      ]);
      ...
      // Ordenar repositórios por estrelas para destacar os mais populares
      ...
      const pageData: HomePageData = {
        page: {
          ...
          highlightProjects: sortedRepos.slice(0, 3).map(repo => ({
            slug: repo.name,
            thumbnail: {
            ...
            pageThumbnail: { url: repo.owner.avatar_url },
            sections: [],
            description: {
              raw: { children: [] },
